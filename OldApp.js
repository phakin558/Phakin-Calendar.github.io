...
export default defineComponent({
  setup() {
    ...
    // ===== Homework: subject ต้องมาจากตารางเรียนเท่านั้น =====
    const subjectOptions = computed(() => {
      const seen = new Map()
      timetable.classes.forEach(c => { if (c.subject && !seen.has(c.subject)) seen.set(c.subject, c.color) })
    ...
    function emptyTF() { return { subject:'', title:'', dueDate:'', noDueDate:false, notes:'', color:'' } }

    function onSubjectChange() {
      const found = subjectOptions.value.find(s => s.subject === tf.value.subject)
      tf.value.color = found ? found.color : ''
    }

    function submitTask() {
      if (!tf.value.title || !tf.value.subject) return
      const payload = {
        subject: tf.value.subject,
        title: tf.value.title,
        dueDate: tf.value.noDueDate ? '' : tf.value.dueDate,
        notes: tf.value.notes,
        color: tf.value.color,
      }
      editingTaskId.value
        ? homework.updateTask(editingTaskId.value, payload)
        : homework.addTask(payload)
      cancelTF()
    }
    function editTask(t) {
      editingTaskId.value = t.id
      tf.value = { subject:t.subject, title:t.title, dueDate:t.dueDate, noDueDate: !t.dueDate, notes:t.notes, color:t.color||'' }
      showTaskForm.value = true
    }
    function cancelTF() { showTaskForm.value = false; editingTaskId.value = null; tf.value = emptyTF() }

    // ===== Sort & Filter การบ้าน =====
    const hwSort = ref('asc')     // 'asc' | 'desc'
    const hwFilter = ref('all')   // 'all' | 'overdue' | 'noDate'

    const displayedPending = computed(() => {
      let list = [...homework.pending]
      if (hwFilter.value === 'overdue') list = list.filter(t => isOverdue(t.dueDate))
      else if (hwFilter.value === 'noDate') list = list.filter(t => !t.dueDate)

      list.sort((a, b) => {
        const av = a.dueDate || (hwSort.value === 'asc' ? '9999-99-99' : '0000-00-00')
        const bv = b.dueDate || (hwSort.value === 'asc' ? '9999-99-99' : '0000-00-00')
        return hwSort.value === 'asc' ? av.localeCompare(bv) : bv.localeCompare(av)
      })
      return list
    })
    ...
    <section v-if="activeTab === 'homework'" class="narrow">
      ...
      <div class="form-card" v-if="showTaskForm">
        ...
        <div class="form-grid">
          ...
          <div class="field">
            <label>วิชา *</label>
            <select v-model="tf.subject" @change="onSubjectChange">
              <option value="" disabled>-- เลือกวิชาจากตารางเรียน --</option>
              <option v-for="s in subjectOptions" :key="s.subject" :value="s.subject">{{ s.subject }}</option>
            </select>
          </div>
          <div class="field">
            <label>กำหนดส่ง</label>
            <input type="date" v-model="tf.dueDate" :disabled="tf.noDueDate">
            <label class="checkbox-row"><input type="checkbox" v-model="tf.noDueDate"> ไม่ระบุวันกำหนดส่ง</label>
          </div>
      ...
      <div class="hw-toolbar">
        <div class="field">
          ...
          <select v-model="hwSort">
            <option value="asc">ใกล้ถึงก่อน</option>
            <option value="desc">ไกลสุดก่อน</option>
          </select>
        </div>
        <div class="field">
          <label>กรองรายการ</label>
          <select v-model="hwFilter">
            <option value="all">ทั้งหมด</option>
            <option value="overdue">เลยกำหนดส่ง</option>
            <option value="noDate">ไม่ระบุวัน</option>
          </select>
        </div>
      </div>
     ...
     :style="{ borderLeftColor: t.color || '#615757', '--task-color': t.color || '#615757' }">
          ...
          <div class="task-info">
            ...
            <span class="task-meta">
              ...
              <span :class="{ 'overdue-text': isOverdue(t.dueDate) }">📅 {{ t.dueDate ? fmtDate(t.dueDate) : 'ไม่ระบุวันกำหนดส่ง' }}</span>
            </span>
